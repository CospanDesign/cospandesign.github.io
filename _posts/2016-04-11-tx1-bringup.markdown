...
Essentially the the path from 'power on' to CLI involves a lot of disjointed steps that are confusing. Most of this was learned from these three resources:
  ...
  * [Developer User Guide Documentation](http://developer.nvidia.com/embedded/dlc/l4t-documentation-23-2)
...
To simplify this discussion I'm going to create a board called **quokka** and focus on developing for the 23.2 L4T, this may change later when NVIDIA releases newer builds.
...
Make sure to have all the required build toolchains installed:

{% highlight bash %}
echo "Install 32-bit ARM Cross Compiler"
sudo apt-get install gcc-arm-linux-gnueabi
echo "Install 64-bit ARM Cross Compiler"
sudo apt-get install gcc-aarch64-linux-gnu
{% endhighlight %}


To simplify things we'll make a directory where we can store all the output files from the pinmux called 'quokka_config_files'

{% highlight bash %}
mkdir -p ~/Projects/tx1/quokka_config_files
{% endhighlight %}
...
Extract the [Developer User Guide Documentation](http://developer.nvidia.com/embedded/dlc/l4t-documentation-23-2) to a docs directory

{% highlight bash %}
mkdir -p ~/Projects/tx1/docs
cd ~/Projects/tx1/docs
cp ~/Downloads/Tegra_Linux_Driver_Package_Documents_R23.2.tar .
tar xvf Tegra_Linux_Driver_Package_Documents_R23.2.tar
{% endhighlight %}
...
{% highlight bash %}
cd ~/Projects/tx1/
mv ~/Downloads/Tegra210_Linux_R23.2.0_armhf.tbz2 .
tar xvf Tegra210_Linux_R23.2.0_armhf.tbz2
{% endhighlight %}
...
mv ~/Downloads/Tegra_Linux_Sample-Root-Filesystem_R23.2.0_armhf.tbz2 .
sudo tar xf Tegra_Linux_Sample-Root-Filesystem_R23.2.0_armhf.tbz2
rm Tegra_Linux_Sample-Root-Filesystem_R23.2.0_armhf.tbz2
{% endhighlight %}
...
./source_sync.sh -k tegra-l4t-r23.2.update-01
{% endhighlight %}
...
{% highlight bash %}
cd ~/Projects/tx1/Linux_for_Tegra
./source_sync.sh -u tegra-l4t-r23.2
{% endhighlight %}
...
The spreadsheet will also generate a dtsi file that will be read by the kernel later on so we need to copy it to our 'quokka_config_files' folder we created earlier

{% highlight bash %}
cd ~/Projects/tx1/quokka_config_files
...
supported_boards = {
    ...
    'quokka-2180': {
        # Custom board
        'filename': '../quokka_config_files/quokka-2180.csv',
        'rsvd_base': 0,
        'soc': 'tegra210',
    },
    'p2571': {
...
This will output the header file to stdout so you will need to redirect the output to a file. For reasons I'll explain later call this new file: pinmux-config-quokka-2180.h


{% highlight bash %}
cd tegra-pinmux-scripts
./board-to-uboot.py quokka-2180 > pinmux-config-quokka-2180.h
{% endhighlight %}
...
When the SOC turns on the boot processor wakes up and reads the stae of both the fuses and straps.
...
<pre>
/* High-level configuration options */
#define V_PROMPT			"Tegra210 (P2371-quokka) # "
...
Inside this new directory are a some files to modify
  ...
  * pinmux-config-p2371-2180.h  Our Pinmux Configuration File
...
echo "Change the filenames to reflect our board"
mv p2371-2180.c quokka-2180.c
mv pinmux-config-p2371-2180.h pinmux-config-quokka-2180.h
...
The pinmux-config-p2371-2180.h file is incorrec we need to replace it with the one we generated earlier or we can just pipe the output of the pinmux script to this file:
...
When selecting an SOC like the Tegra 210 you the build configuration allows you to select certain compatible boards. We need to modify that configuration script to allow our board to be selected.
...
The flash tools are configured to install the bootloader and kernel to the board.

  * Copy and modify exiting extlinux.conf\* files to describe medium dependent boot configuration to the bootloader
...
The board configuration file populates constants that are board depenedent. These constants describe the ID of the processor the deisplay and the power management unit.
